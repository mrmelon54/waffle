...
export default class SchemaObject {
  ...
  static parse(ctx: OpenApiContext, v: any): Promise<Optional<SchemaObject>> {
    if (v === null || v === undefined) return StaticOptional.empty();
    let o = new SchemaObject();
    o.$$raw = v;
    o.$$ctx = ctx;
    o.$ref = StaticOptional.full(v.$ref);
    if (o.$ref.isFull()) return ReferenceOptional.generateCtx(ctx, o.$ref.get(), SchemaObject.parse);
