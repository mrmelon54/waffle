...
export default class OpenApiObject {
  ...
  lookup(ref: string[]): Optional<any> {
    let v: any = this;
    for (let i of ref) {
      if (v.isEmpty !== undefined) {
        if (v.isEmpty() || v.hasError()) return StaticOptional.emptyWithError(`Lookup failed: ${v.errorReason() ?? "No reason"}`);
        if (v.isFull()) v = v.get();
      }
      if (v.__proto__ === Map.prototype) v = v.get(i);
      else v = v[i];
      if (v === null || v === undefined) return StaticOptional.emptyWithError(`Lookup failed: value was null or undefined`);
    }
    return StaticOptional.full(v);
  }
}
