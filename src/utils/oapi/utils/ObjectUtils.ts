...
// export type CtxParser<T> = (ctx: OpenApiParser, v: any) => Promise<T>;

// export function parseArray<T>(v: any, parser: Parser<T>): Promise<T[]> {
//   return parseCtxArray(emptyContext, v, (_, x) => parser(x));
// }

// export async function parseCtxArray<T>(ctx: OpenApiParser, v: any, parser: CtxParser<T>): Promise<T[]> {
...
// export async function parseCtxMap<T, U>(ctx: OpenApiParser, v: any, parser: CtxParser<U>): Promise<Map<T, U>> {
